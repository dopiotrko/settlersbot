...
class Adventure:
    ...
    def c_move_army(self):
        ...
        loc = my_pygui.locateOnScreen('resource/c_move.png',
                                      ...
                                      confidence=0.90)
        if loc:
            log.info('move army found - moveing')
            my_pygui.click(loc.get())
            return True
        else:
...
@my.send_explorer_while_error
def run_arctic(adv_name='arktyczna', delay=5, gap=10, start=0, stop=999):
    adv = Adventure(adv_name)
    adv.start_adventure(adv_name, delay=delay)
    my.wait(20, 'loading')
    adv.focus()
    my_pygui.hotkey('alt', '1')
    my.wait(10)
    adv.c_load_army()
    my.wait(15)
    adv.c_send_army()
    adv.send_explorer_by_client(500)
    my.wait(100)
    adv.go_to_adventure(gap)
    my.wait(20, 'collect')
    my_pygui.hotkey('ctrl', 'F4')
    my.wait(15)
    my_pygui.hotkey('alt', '2')
    my.wait(10)
    adv.c_move_army()
    my.wait(60, 'attack 1')
    my_pygui.hotkey('alt', '3')
    my.wait(10)
    adv.c_attack_army()
    my.wait(120, 'attack 2')
    my_pygui.hotkey('alt', '4')
    my.wait(10)
    adv.c_attack_army()
    my.wait(30, 'attack 3')
    my_pygui.hotkey('alt', '5')
    my.wait(10)
    adv.c_attack_army()
    my.wait(60, 'attack 4')
    my_pygui.hotkey('alt', '6')
    my.wait(10)
    adv.c_attack_army()
    my.wait(110, 'attack 5')
    my_pygui.hotkey('alt', '7')
    my.wait(10)
    adv.c_attack_army()
    adv.confirm_task(200, ending=False)
    my.wait(15, 'collect')
    my_pygui.hotkey('ctrl', 'F4')
    my.wait(30)
    adv.confirm_task(5, ending=True)
    my.wait(30)


@my.send_explorer_while_error
def run_azz(adv, delay=5, gap=7*60, start=0, stop=999):
    adv.start_adventure('zaskocze', delay=delay)
    my.wait(20, 'loading')
    adv.focus()
    my_pygui.hotkey('ctrl', 'alt', 'q')
    my.wait(10, 'loading army')
    adv.c_load_army()
    my.wait(15, 'sending army')
    adv.c_send_army()
    adv.send_explorer_by_client(gap)
    adv.go_to_adventure(100)
    my.wait(20, 'collect')
    my_pygui.hotkey('ctrl', 'F4')
    my.wait(20, 'move to star')
    my_pygui.hotkey('ctrl', 'shift', 'p')
    my.wait(30, "land 1")
    my_pygui.hotkey('ctrl', 'alt', 'w')
    my.wait(10)
    adv.c_move_army()
    my.wait(35, 'move to init pos')
    my_pygui.hotkey('ctrl', 'alt', 'e')
    my.wait(10)
    adv.c_move_army()
    my.wait(30, 'land 2')
    my_pygui.hotkey('ctrl', 'alt', 'r')
    my.wait(10, 'move confirm')
    adv.c_move_army()
    my.wait(100, 'load for first attack')
    my_pygui.hotkey('ctrl', 'alt', 't')
    my.wait(10, 'load confirm')
    adv.c_load_army()
    my.wait(20, 'first attack')
    adv.c_attack_army()
    my.wait(400, 'refresh after first attack')
    my_pygui.hotkey('F2')
    my.wait(10, 'load for second attack')
    my_pygui.hotkey('ctrl', 'alt', 'y')
    my.wait(10, 'confirm load')
    adv.c_load_army()
    my.wait(10, 'move for second attack')
    adv.c_move_army()
    my.wait(115, 'second attack')
    my_pygui.hotkey('ctrl', 'alt', 'y')
    my.wait(10, 'confirm attack')
    adv.c_attack_army()
    my.wait(250, 'refresh after second attack')
    my_pygui.hotkey('F2')
    my.wait(10, 'move for last pos')
    my_pygui.hotkey('ctrl', 'alt', 'u')
    my.wait(10, 'confirm move')
    adv.c_move_army()
    my.wait(190, 'load for third attack')
    my_pygui.hotkey('ctrl', 'alt', 'i')
    my.wait(10, 'confirm load')
    adv.c_load_army()
    my.wait(17, 'confirm third attack')
    adv.c_attack_army()
    my.wait(270, 'refresh after second attack')
    my_pygui.hotkey('F2')
    my.wait(10, 'load for fourth attack')
    my_pygui.hotkey('ctrl', 'alt', 'o')
    my.wait(10, 'confirm load')
    adv.c_load_army()
    my.wait(17, 'confirm fourth attack')
    adv.c_attack_army()
    adv.confirm_task(355, ending=False)
    my.wait(30)
